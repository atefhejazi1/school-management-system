...
class AddParent extends Component
{
    ...
    {
        // start transaction to handle the multiple insertions and updates in a single database transaction
        DB::beginTransaction();
        ...
        try {
            ...
            if (!empty($this->photos)) {
                foreach ($this->photos as $file) {
                    $file->storeAs('attachments/parents/' . $this->National_ID_Father, $file->getClientOriginalName(), 'parent_attachments');

                    $image = new Image();
                    $image->filename = $file->getClientOriginalName();
                    $image->imageable_id = $My_Parent->id;
                    $image->imageable_type = My_Parent::class;
                    $image->save();
                }
            }
            ...
            // rollback the transaction in case of any failure during insertions or updates
            DB::rollback();
    ...
    {
        $p =  My_Parent::findOrFail($id);

        $folderPath = storage_path('app/parent_attachments') . '/' . $p->National_ID_Father;
